import React from 'react'
import styles from "../../../styles/styles";
const Sponsord = () => {
  return (
    <div  className={`${styles.section} hidden sm:block bg-white py-10 px-5   mb-12 cursor-pointer rounded-xl p-3 shadow-sm`}>
      <div className='flex justify-between w-full'>
         <div className="flex items-start">
          <img
            src="https://tse1.mm.bing.net/th/id/OIP.AXnhRHG7scN3qxlS0akrKAAAAA?pid=Api&P=0&h=220"
            alt=""
            style={{width:"150px", objectFit:"contain"}}
          />
        </div>
        <div className="flex items-start">
          <img
            src="https://tse4.mm.bing.net/th/id/OIP.HgH-NjiOdFOrkmwjsZCCfAHaHl?pid=Api&P=0&h=220&h=220"
            style={{width:"150px", objectFit:"contain"}}
            alt=""
          />
        </div>
        <div className="flex items-start">
          <img
            src="https://tse1.mm.bing.net/th/id/OIP.jbvAFtAbO6ip4MMHHR5y0gHaEK?pid=Api&P=0&h=220"
            style={{width:"150px", objectFit:"contain"}}
            alt=""
          />
        </div>
        <div className="flex items-start">
          <img
            src="https://tse2.mm.bing.net/th/id/OIP.6Gui94cxC_Bzu91WGfy5fwHaE8?pid=Api&P=0&h=220"
            style={{width:"150px", objectFit:"contain"}}
            alt=""
          />
        </div>
        <div className="flex items-start">
          <img
            src="data:image/png;base64,"
            style={{width:"150px", objectFit:"contain"}}
            alt=""
          />
        </div>
      </div>
    </div>
  )
}

export default Sponsord